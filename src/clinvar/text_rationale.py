...
class Encoder(nn.Module):
    ...
    def forward(self, inputs, lengths=None, z_mask=None):
        ...
        output, hidden = self.encode(packed_emb)  # embed_input
    ...
    def encode(self, inputs, lengths):
        ...
        output, hidden = self.forward(inputs, lengths)
        preds = self.out(output)

        return preds

    def get_encode_loss(self, inputs, lengths, y_labels):
        # called by encoder pretraining
        preds = self.encode(inputs, lengths)
...
class SampleGenerator(nn.Module):
    ...
    def forward(self, inputs, lengths=None):
        ...
        output, hidden = self.generator(packed_emb)  # embed_input
    ...
    def compute_sparsity_penalty(self, z_mask, sparsity=4e-4, coherent=2):
        ...
        sparsity_coherence_cost = torch.mean(z_sum) * sparsity + torch.mean(z_diff) * coherent
        sparsity_coherence_cost_vec = z_sum * sparsity + z_diff * coherent
    ...
    def get_loss(self, encoder_loss_vec, z_mask_logits, z_mask_prob_dist, z_mask):
...
def train_model(model, optimizer, train_iter, valid_iter, test_iter, max_epoch):
    model.train()
    criterion = nn.CrossEntropyLoss()
