...
from attn_module import Encoder
...
class Model(nn.Module):
    def __init__(self, vocab, emb_dim=100, hidden_size=256, depth=1, nclasses=5,
        ...
        # ha....I don't see where n_max_seq is actually used...
        self.encoder = Encoder(src_vocab_size, n_max_seq=200,
                               n_layers=depth, n_head=3, d_k=64, d_v=64,
                               d_word_vec=emb_dim, d_model=hidden_size,
                               d_inner_hid=hidden_size, dropout=0.2)
        self.encoder.src_word_emb.weight.data.copy_(vocab.vectors)
        d_out = hidden_size
        self.out = nn.Linear(d_out, nclasses)

        self.universal_query = nn.Parameter(torch.randn(hidden_size, hidden_size))
    ...
    def forward(self, inputs, lengths):
        ...
        pos = np.zeros((len(lengths_list), max_len), dtype='int64')
        for i, l in enumerate(lengths_list):
            for l_pos in xrange(l):
                pos[i, l_pos] = l_pos + 1

        t_pos = torch.from_numpy(pos)
        t_pos_var = Variable(t_pos, requires_grad=False)  # no update on positional variable
        t_pos_var = move_to_cuda(t_pos_var)

        # (T, batch_size, hidden_size)
        output = self.encoder(inputs, t_pos_var)

        # (T, batch_size)
        keys = torch.sum(self.universal_query.view(1, -1, self.hidden_size) * output, 2)
